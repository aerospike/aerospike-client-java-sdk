...
> **AEL v2** — Quick overview: [cheat-sheet.md](cheat-sheet.md). See [DECISIONS.md](DECISIONS.md), [SOURCES.md](SOURCES.md), [open-issues.md](open-issues.md). **Last reviewed:** 2026-05-20.

## Normative v2 update: nesting restriction

**Nested sub-expressions are disallowed:** a sub-expression must not appear inside another sub-expression used in a filter or path argument. Examples in this document that imply arbitrarily deep nested filters are **not** v2-conformant unless rewritten (for example using `let`).
...
**What stays literal:** Selector **dimension** markers (`@`, `=`, `#`, `!`, …), range punctuation inside **`{…}`** / **`[…]`**, and **regex** patterns in **`=~`** stay literal as defined in [path-and-selectors.md](path-and-selectors.md). **Static types** on reads are written with a **`:T`** suffix on the path (including after a selector), e.g. **`$.l.[0]:INT`**, **`$.m.k:INT`**, **`$.key():INT`** — not as **`type:`** parameters inside a separate **`get()`** call.
...
Cannot determine whether '{=($.x)}' is a single-value lookup or a value-list lookup.
  If $.x is a single value: $.x:INT
  If $.x is a list:         $.x:LIST
```
...
2. **Explicit annotation:** `$.m.{($.idx)}:INT` → output is INTEGER
3. **Propagation:** if the output is compared to something whose type is already known

If the output type cannot be resolved, the parser raises an error suggesting a **`:`** type suffix on the path (e.g. **`$.m.{($.idx)}:INT`**).
