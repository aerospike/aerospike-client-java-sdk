...
public class LatencyManagerAlternate implements LatencyManager {
    ...
    public LatencyManagerAlternate(int columns, int bitShift, String units) {
    ...
    public void printResults(PrintStream stream, String prefix) {
        ...
        printColumn(stream, 1, sum, buckets[0].header, array[0]);
        printColumn(stream, 1, sum, buckets[1].header, array[1]);

        int limit = 1;

        for (int i = 2; i < array.length; i++) {
            limit <<= bitShift;
            printColumn(stream, limit, sum, buckets[i].header, array[i]);
        }
        stream.print(" total(");
        stream.print(sum);
        stream.print(')');
    ...
    private void printColumn(PrintStream stream, int limit, double sum, String header, int count) {
        ...
        double percent = (count > 0) ? count * 100.0 / sum : 0.0;
