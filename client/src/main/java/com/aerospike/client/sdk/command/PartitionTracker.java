...
public final class PartitionTracker {
    ...
    private PartitionTracker(Command cmd, Node[] nodes, PartitionFilter filter, long maxRecords) {
        this(cmd, nodes.length, filter, maxRecords);
    }

    private PartitionTracker(Command cmd, int nodeCapacity, PartitionFilter filter, long maxRecords) {
        ...
        if (!(filter.begin >= 0 && filter.begin < Node.PARTITIONS)) {
            throw AerospikeException.toException(ResultCode.PARAMETER_ERROR, "Invalid partition begin " + filter.begin +
                ". Valid range: 0-" + (Node.PARTITIONS - 1));
        }

        if (filter.count <= 0) {
            throw AerospikeException.toException(ResultCode.PARAMETER_ERROR, "Invalid partition count " + filter.count);
        }

        if (filter.begin + filter.count > Node.PARTITIONS) {
            throw AerospikeException.toException(ResultCode.PARAMETER_ERROR, "Invalid partition range (" + filter.begin +
                ',' + filter.count + ')');
        }
    ...
    private void setMaxRecords(long maxRecords) {
        if (maxRecords < 0) {
            throw AerospikeException.toException(ResultCode.PARAMETER_ERROR, "Invalid maxRecords: " + maxRecords);
        }
    ...
    private void init(Command cmd) {
        sleepBetweenRetries = cmd.sleepBetweenRetries;
        socketTimeout = cmd.socketTimeout;
        totalTimeout = cmd.totalTimeout;
        ...
        if (replica == Replica.RANDOM) {
            throw AerospikeException.toException(ResultCode.PARAMETER_ERROR, "Invalid replica: " + replica.toString());
        }
    }
    ...
    public List<NodePartitions> assignPartitionsToNodes(Cluster cluster, String namespace) {
        ...
        HashMap<String,Partitions> map = cluster.getPartitionMap();
    ...
    public boolean isComplete(Cluster cluster, Command cmd) {
        return isComplete(cmd, nodePartitionsList);
    }

    public boolean isComplete(Command cmd, List<NodePartitions> nodePartitionsList) {
        long recCount = 0;
        ...
        if (iteration > cmd.maxRetries) {
            ...
            AerospikeException ae = AerospikeException.toException(last.getResultCode(), sb.toString());
