...
public class BehaviorYamlLoader {
    ...
    private static SystemSettings convertToSystemSettings(BehaviorYamlConfig.SystemSettingsConfig config) {
        ...
        // Apply transactions settings
        if (config.getTransactions() != null) {
            BehaviorYamlConfig.SystemTransactionsConfig transactionsConfig = config.getTransactions();
            builder.transactions(ops -> {
                if (transactionsConfig.getImplicitBatchWriteTransactions() != null) {
                    ops.implicitBatchWriteTransactions(transactionsConfig.getImplicitBatchWriteTransactions());
                }
                if (transactionsConfig.getSleepBetweenAttempts() != null) {
                    ops.sleepBetweenAttempts(transactionsConfig.getSleepBetweenAttempts());
                }
