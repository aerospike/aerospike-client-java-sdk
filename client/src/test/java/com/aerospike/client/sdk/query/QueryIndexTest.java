...
public class QueryIndexTest extends ClusterTest {
    ...
    public void createDrop() {
        ...
        try {
            session.dropIndex(args.set, indexName).waitTillComplete();
        }
        ...
        session.createIndex(args.set, indexName, binName, IndexType.INTEGER,
    ...
    public void setIndexCreateDrop() {
        ...
        assumeTrue(serverVersion.isGreaterOrEqual(Version.SERVER_VERSION_8_1_2),
            "Set index tests require server version 8.1.2 or later");

        // Drop set index if it already exists.
        try {
            session.dropIndex(args.set, setIndexName).waitTillComplete();
        }
        catch (AerospikeException ae) {
            if (ae.getResultCode() != ResultCode.INDEX_NOTFOUND) {
                throw ae;
        ...
        session.createIndex(args.set, setIndexName)
            .waitTillComplete();
        ...
        for (Node node : nodes) {
            String cmd = IndexTask.buildExistsCommand(args.namespace, setIndexName, node.getVersion());
            String response = Info.request(node, cmd);
        ...
        for (Node node : nodes) {
            String cmd = IndexTask.buildStatusCommand(args.namespace, setIndexName, node.getVersion());
            String response = Info.request(node, cmd);
            int code = Info.parseResultCode(response);
